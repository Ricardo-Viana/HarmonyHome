...
function Rooms(){
    ...
    useEffect(() => {
        if(loginInfo){
            ...
            setCountTasks(tasks.filter(task => task.id_user === loginInfo.id && task.id_house === parseInt(id) && !task.done).length)
        }
        else{
            navigate("/")
        }
        ...
        <Container sx={{display: "flex", flexDirection: "row"}}>
            ...
            {userHouseRooms &&
                userHouseRooms.map((comodo, index) => (
                    ...
                    <Typography variant="h6" sx={{color: "secondary.main"}}> ({`${(tasks.filter(task => task.id_user === loginInfo.id && task.id_house === parseInt(id) && task.id_room === comodo.id && !task.done)).length}`})</Typography> 
                    <IconButton aria-label="Add" sx={{color: "primary.contrastText"}} onClick={() => navigate(`/task/${comodo.id}/${id}`)}>
                        <AddCircleIcon />
                    </IconButton>
                    <Typography variant="h6" sx={{color: "primary.contrastText"}}> {comodo.name} </Typography>                    
                    </Box>
                ))
            }
